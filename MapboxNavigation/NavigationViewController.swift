...
extension NavigationViewController: RouteMapViewControllerDelegate {
    public func navigationMapView(_ mapView: NavigationMapView, didSelect waypoint: MapboxDirections.Waypoint) {}
...
extension NavigationViewController: RouteControllerDelegate {
    ...
    public func routeController(_ routeController: RouteController, didUpdate locations: [CLLocation]) {
        ...
        guard let waypoint = routeController.routeProgress.currentLeg.destination else { return }
		
        let shouldPreventReroutesWhenArrivingAtWaypoint = routeController.delegate?.routeController(routeController, shouldPreventReroutesWhenArrivingAt: waypoint) ?? true
