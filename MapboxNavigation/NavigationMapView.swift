...
open class NavigationMapView: MLNMapView, UIGestureRecognizerDelegate {
    ...
    public dynamic var trafficUnknownColor: UIColor = .trafficUnknown
    public dynamic var trafficLowColor: UIColor = .trafficLow
    public dynamic var trafficModerateColor: UIColor = .trafficModerate
    public dynamic var trafficHeavyColor: UIColor = .trafficHeavy
    public dynamic var trafficSevereColor: UIColor = .trafficSevere
    public dynamic var routeLineColor: UIColor = .defaultRouteLine
    public dynamic var routeLineAlternativeColor: UIColor = .defaultRouteLineAlternative
    public dynamic var routeLineCasingColor: UIColor = .defaultRouteLineCasing
    public dynamic var routeLineCasingAlternativeColor: UIColor = .defaultRouteLineCasingAlternative
    public dynamic var maneuverArrowColor: UIColor = .defaultManeuverArrow
    public dynamic var maneuverArrowStrokeColor: UIColor = .defaultManeuverArrowStroke
    ...
    public func showWaypoints(_ route: Route, legIndex: Int = 0) {
        ...
        let waypoints: [Waypoint] = Array(route.legs.compactMap(\.destination).dropLast())
        ...
        if waypoints.count > 2 { // are we on a multipoint route?
    ...
    public func addArrow(route: Route, legIndex: Int, stepIndex: Int) {
        ...
        guard let trimmedPolylineBegin = polyline.trimmed(from: maneuverCoordinate, distance: -shaftLength) else { return }
        guard let trimmedPolylineEnd = polyline.trimmed(from: maneuverCoordinate, distance: shaftLength) else { return }
		
        let shaftCoordinates = Array(trimmedPolylineBegin.coordinates.reversed() + trimmedPolylineEnd.coordinates.suffix(from: 1))
    ...
    private func waypoints(on routes: [Route], closeTo point: CGPoint) -> [Waypoint]? {
        ...
        let multipointRoutes = routes.filter { $0.legSeparators.count >= 3 }
        guard multipointRoutes.count > 0 else { return nil }
        let waypoints = multipointRoutes.flatMap(\.legSeparators).compactMap { $0 }
    ...
    private func routes(closeTo point: CGPoint) -> [Route]? {
        ...
        guard let routes = self.routes?.filter({ $0.shape?.coordinates.count ?? 0 > 1 }) else { return nil }
        ...
        let closest = routes.sorted { left, right -> Bool in
            ...
            let leftLine = LineString(left.shape!.coordinates)
            let rightLine = LineString(right.shape!.coordinates)
        ...
        let candidates = closest.filter {
            let closestCoordinate = LineString($0.shape!.coordinates).closestCoordinate(to: tapCoordinate)!.coordinate
    ...
    func shape(for routes: [Route], legIndex: Int?) -> MLNShape? {
        ...
        for route in routes.suffix(from: 1) {
            let polyline = MLNPolylineFeature(coordinates: route.shape!.coordinates, count: UInt(route.shape!.coordinates.count))
    ...
    func addCongestion(to route: Route, legIndex: Int?) -> [MLNPolylineFeature]? {
        ...
        for (index, leg) in route.legs.enumerated() {
            ...
            guard let legCongestion = leg.segmentCongestionLevels, legCongestion.count < coordinates.count else {
                return [MLNPolylineFeature(coordinates: route.shape!.coordinates, count: UInt(route.shape!.coordinates.count))]
            }
    ...
    func shape(forCasingOf route: Route, legIndex: Int?) -> MLNShape? {
        ...
        for (index, leg) in route.legs.enumerated() {
            let legCoordinates: [CLLocationCoordinate2D] = leg.steps.flatMap { $0.shape!.coordinates }
